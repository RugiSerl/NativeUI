...
#include "utils.hpp"
#include <algorithm>
UISelectablePanel::UISelectablePanel(raylib::Rectangle r, Anchor2 a, UIStylebox normalStylebox, UIStylebox selectedStylebox) : UIPanel(r, a, normalStylebox), selectedStylebox(selectedStylebox) {
    selected = false;
}
...
void UISelectablePanel::onSelected() {
    if (this->parent == NULL) return; // root case
    // in case of overlapping, happening only between two siblings because of the scissor mode restricting area for children
    // put this element at the end - on top
    ...
    for (int i = 0; i<this->parent->GetChildrenCount()-1; i++) { // we spare this object, at the back of the vector
